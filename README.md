# CodeAgent：无框架、自实现的代码智能体

CodeAgent 是一个面向 Python 项目的代码智能体。项目不使用 LangChain、AutoGen、OpenAI Agents SDK 等 Agent 框架，任务规划、主循环、工具调用、状态迁移、代码编辑、验证和失败恢复均由项目自行实现。

项目关注的不是“让模型直接改代码”，而是让模型在程序约束下自主决策，并要求所有成功结论都由真实运行证据支持。
